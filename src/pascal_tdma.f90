...
!>              The modified Thomas algorithm by Laszlo et al.(2016) and newly designed communication scheme 
!>              are used to reduce the communication overhead in solving many tridiagonal systems.
...
!>                  Each computing core transforms the partitioned sub-matrices in the tridiagonal systems 
!>                  of equations into the modified forms by applying the modified Thomas algorithm.
!>              (2) Construct reduced tridiagonal systems from the modified sub-matrices
!>                  The reduced tridiagonal systems are constructed by collecting the first and last row 
!>                  of the modified sub-matrices from each cores using MPI_Ialltoallw.
...
!>                  The solutions of reduced tridiagonal systems in Step 3 are distributed to each core 
!>                  using MPI_Ialltoallw. This communication is an exact inverse of communication in Step 2.
!>              (5) Update the other unknowns in the modified tridiagonal systems
!>                  The remaining unknowns of the modified sub-matrices in Step 1 are solved in each computing core 
